...
services:
  aobobo:
    ...
    ports:
      - 80:80
    volumes:
      # - ./config.js:/home/wwwroot/html/config.js:ro # 自定义运行时配置
      # - ./style.css:/home/wwwroot/html/style.css:ro # 自定义样式文件
      # - ./favicon.svg:/home/wwwroot/html/favicon.svg:ro # 自定义站点图标
```

`NEZHA` 需要以 `/` 结尾，例如 `http://nezha-dashboard/`。
