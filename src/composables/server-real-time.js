...
export default (params) => {
  ...
  const duration = computed(() => {
    if (props.info?.Host?.BootTime) {
      ...
      const endTime = lastActive || currentTime.value;
      const data = dateUtils.duration2(props.info.Host.BootTime * 1000, endTime);
      if (data.days > 0) {
        return {
          value: data.days,
          unit: data.$unit.day,
    ...
    switch (key) {
      ...
      case 'conns':
        return {
          ...
          show: validate.isSet(props.info.State?.TcpConnCount)
            || validate.isSet(props.info.State?.UdpConnCount),
          data: {
            tcp: {
      ...
      case 'L-A-P':
        return {
          ...
          show: validate.isSet(props.info.State?.Load1)
            || validate.isSet(props.info.State?.ProcessCount),
        };
      case 'T-A-U':
        return {
          key,
