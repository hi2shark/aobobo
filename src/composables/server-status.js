...
function calcUsePercent(usedRaw, totalRaw) {
  const used = usedRaw || 0;
  const total = totalRaw || 1;
  return Number(((used / total) * 100).toFixed(2)) || 0;
}
...
export default (params) => {
  ...
  const useMemAndTotalMem = computed(() => {
    const memUsed = props.info?.State?.MemUsed || 0;
    const memTotal = props.info?.Host?.MemTotal || 1;
    const used = hostUtils.calcBinary(memUsed);
    const total = hostUtils.calcBinary(memTotal);
    const usePercent = calcUsePercent(memUsed, memTotal);
    return {
      used,
      total,
      usePercent,
  ...
  const useSwapAndTotalSwap = computed(() => {
    ...
    const swapUsed = props.info?.State?.SwapUsed || 0;
    const swapTotal = props.info?.Host?.SwapTotal || 1;
    const used = hostUtils.calcBinary(swapUsed);
    const total = hostUtils.calcBinary(swapTotal);
    const usePercent = calcUsePercent(swapUsed, swapTotal);
    return {
      used,
      total,
      usePercent,
  ...
  const useDiskAndTotalDisk = computed(() => {
    const diskUsed = props.info?.State?.DiskUsed || 0;
    const diskTotal = props.info?.Host?.DiskTotal || 1;
    const used = hostUtils.calcBinary(diskUsed);
    const total = hostUtils.calcBinary(diskTotal);
    const usePercent = calcUsePercent(diskUsed, diskTotal);
    return {
      used,
      total,
      usePercent,
